#!/usr/bin/env python3
"""Build Dashboard HTML with embedded JSON (robust script-tag approach)."""
import json
...
# The only dangerous sequence is </script> which must be escaped
data_json_escaped = data_json.replace("</", "<\\/")
models_json_escaped = models_json.replace("</", "<\\/")
...
<body>
<header><h1>论文速读能力评测 国产大模型对比</h1><p>SiliconFlow API | 5 Models x 7 Tasks | <span id="tc">-</span> calls | {now_str}</p></header>
<main>
<div class="kpi-row" id="kpi"></div>

<section>
<div class="sh" onclick="this.nextElementSibling.classList.toggle('hide')">Score Heatmap <span style="float:right">&#9660;</span></div>
<div class="sb" id="matrix"></div>
</section>

<section>
<div class="sh" onclick="this.nextElementSibling.classList.toggle('hide')">Radar: Text Task Capabilities <span style="float:right">&#9660;</span></div>
<div class="sb"><div class="chart-box"><canvas id="radar"></canvas></div></div>
</section>

<section>
<div class="sh" onclick="this.nextElementSibling.classList.toggle('hide')">Performance <span style="float:right">&#9660;</span></div>
<div class="sb">
<div class="g3" id="perf"></div>
<div class="g2" style="margin-top:12px">
<div class="chart-box"><h4 style="text-align:center;color:#8b949e;margin-bottom:8px">Text Task Latency (s)</h4><canvas id="latBar"></canvas></div>
<div class="chart-box"><h4 style="text-align:center;color:#8b949e;margin-bottom:8px">Text Task Tokens</h4><canvas id="tokBar"></canvas></div>
</div>
<div class="g2" style="margin-top:12px">
<div class="chart-box"><h4 style="text-align:center;color:#8b949e;margin-bottom:8px">Multimodal Latency (s)</h4><canvas id="mmLatBar"></canvas></div>
<div class="chart-box"><h4 style="text-align:center;color:#8b949e;margin-bottom:8px">Multimodal Tokens</h4><canvas id="mmTokBar"></canvas></div>
</div>
</div>
</section>

<section>
<div class="sh" onclick="this.nextElementSibling.classList.toggle('hide')">Hallucination Detection <span style="float:right">&#9660;</span></div>
<div class="sb" id="hall"></div>
</section>

<section>
<div class="sh" onclick="this.nextElementSibling.classList.toggle('hide')">Multimodal Zone <span style="float:right">&#9660;</span></div>
<div class="sb" id="multi"></div>
</section>

<section>
<div class="sh" onclick="this.nextElementSibling.classList.toggle('hide')">Routing Recommendations <span style="float:right">&#9660;</span></div>
<div class="sb" id="route"></div>
</section>
</main>
<footer>SiliconFlow API | Scoring: Accuracy / Completeness / Honesty | Text & Multimodal routed separately</footer>

<script type="application/json" id="eval-data">{data_json_escaped}</script>
<script type="application/json" id="model-stats">{models_json_escaped}</script>
<script>
(function() {{
  "use strict";
  ...
  document.getElementById("kpi").innerHTML =
   "<div class='kpi'><div class='n'>"+models.length+"</div><div class='l'>Models</div></div>"+
   "<div class='kpi'><div class='n'>"+tasks.length+"</div><div class='l'>Tasks (5T+2M)</div></div>"+
   "<div class='kpi'><div class='n'>"+D.length+"</div><div class='l'>Calls ("+okCount+" OK)</div></div>"+
   "<div class='kpi'><div class='n"+(failCount>0?" warn":"")+"'>"+failCount+"</div><div class='l'>Failed ("+frStr+")</div></div>";
  ...
  document.getElementById("perf").innerHTML = sortedModels.map(function(m){{
   var tier = m.avg_latency<15?"Economy":m.avg_latency<60?"Balanced":"Heavy";
   return "<div class='card'><b>"+m.label+"</b><br>Latency: "+m.avg_latency+"s | Tokens: "+m.avg_tokens+"<br>Tier: "+tier+"</div>";
  }}).join("");
  ...
  if(hasCharts){{
    ...
    new Chart(lctx,{{
      ...
      options:{{scales:{{y:{{title:{{display:true,text:"seconds",color:"#c9d1d9"}},ticks:{{color:"#c9d1d9"}},grid:{{color:"#30363d"}}}},x:{{ticks:{{color:"#c9d1d9"}}}}}},plugins:{{legend:{{labels:{{color:"#c9d1d9"}}}}}}}}
    }});
  }}
  ...
  if(hasCharts){{
    var vlmModels = models.filter(function(m){{return m.indexOf("Kimi")>=0||m.indexOf("VL")>=0;}});
    var vlmColors = ["#9c27b0","#ff9800"];
    var mlctx = document.getElementById("mmLatBar").getContext("2d");
    new Chart(mlctx,{{
      type:"bar",
      data:{{labels:multiTasks.map(function(t){{return taskNames[t].substring(0,6);}}),datasets:vlmModels.map(function(m,i){{return{{label:m,data:multiTasks.map(function(t){{var r=D.find(function(x){{return x.model.label===m&&x.task.id===t;}});return r&&r.ok?r.output.latency:0;}}),backgroundColor:vlmColors[i%2]+"88"}};}})}},
      options:{{scales:{{y:{{title:{{display:true,text:"seconds",color:"#c9d1d9"}},ticks:{{color:"#c9d1d9"}},grid:{{color:"#30363d"}}}},x:{{ticks:{{color:"#c9d1d9"}}}}}},plugins:{{legend:{{labels:{{color:"#c9d1d9"}}}}}}}}
    }});
  }}
  ...
  if(hasCharts){{
    var vlmModels2 = models.filter(function(m){{return m.indexOf("Kimi")>=0||m.indexOf("VL")>=0;}});
    var vlmColors2 = ["#9c27b0","#ff9800"];
    var mtctx = document.getElementById("mmTokBar").getContext("2d");
    new Chart(mtctx,{{
      type:"bar",
      data:{{labels:multiTasks.map(function(t){{return taskNames[t].substring(0,6);}}),datasets:vlmModels2.map(function(m,i){{return{{label:m,data:multiTasks.map(function(t){{var r=D.find(function(x){{return x.model.label===m&&x.task.id===t;}});return r&&r.ok&&r.output.usage?r.output.usage.total_tokens:0;}}),backgroundColor:vlmColors2[i%2]+"88"}};}})}},
      options:{{scales:{{y:{{title:{{display:true,text:"tokens",color:"#c9d1d9"}},ticks:{{color:"#c9d1d9"}},grid:{{color:"#30363d"}}}},x:{{ticks:{{color:"#c9d1d9"}}}}}},plugins:{{legend:{{labels:{{color:"#c9d1d9"}}}}}}}}
    }});
  }}
  ...
  var hh = "<h3>Hallucination Detection: Model Honesty</h3><table><tr><th>Model</th><th>Q1</th><th>Q2</th><th>Q3</th><th>Q4</th><th>Q5</th><th>Honesty</th></tr>";
  ...
  hh+="</table><p style='margin-top:8px;color:#8b949e'>Green = honestly said NOT IN ABSTRACT | Red = fabricated info not in paper</p>";
  ...
  multiTaskIds.forEach(function(tid){{
   ...
   if(k&&v){{
    ...
    mh += "<p><b>Qwen-VL-8B is "+speedup+"x faster than Kimi, saves "+savePct+"% tokens</b></p>";
   }}
  }});
  document.getElementById("multi").innerHTML = mh||"<p class='gray'>No multimodal data</p>";

  // Routing
  var rh = "<table><tr><th>Task</th><th>Best Model</th><th>Score</th><th>Latency</th><th>Fallback</th></tr>";
  allTasks.forEach(function(tid){{
   ...
   else{{rh+="<td colspan='4' class='gray'>No available model</td>";}}
   rh += "</tr>";
  }});
  rh += "</table><p style='margin-top:8px;color:#8b949e'>Text tasks and multimodal tasks routed separately. Pure text models cannot handle multimodal.</p>";
...
print(f"Features: JSON-in-script-tag embedding, Chart.js guards, split text/multimodal charts")
