#!/usr/bin/env python3
"""Regenerate Dashboard from scored day1_results.json — v2 with fixed charts"""
import json
...
<div class="chart-box"><h4 style="text-align:center;color:#8b949e;margin-bottom:8px">文本任务 Token 消耗</h4><canvas id="tokBar"></canvas></div>
</div>
<div class="g2" style="margin-top:12px">
<div class="chart-box"><h4 style="text-align:center;color:#8b949e;margin-bottom:8px">多模态延迟对比 (秒)</h4><canvas id="mmLatBar"></canvas></div>
<div class="chart-box"><h4 style="text-align:center;color:#8b949e;margin-bottom:8px">多模态 Token 对比</h4><canvas id="mmTokBar"></canvas></div>
</div>
</div>
</section>
...
<footer>SiliconFlow API . Scoring: Accuracy/Completeness/Honesty . Separate routing for text and multimodal</footer>
...
document.getElementById("perf").innerHTML = sortedModels.map(function(m){{
 ...
 return "<div class='card'><b>"+m.label+"</b><br>Latency: "+m.avg_latency+"s | Tokens: "+m.avg_tokens+"<br>"+tier+"</div>";
}}).join("");
...
new Chart(mtctx,{{
 type:"bar",
 data:{{labels:multiTasks.map(function(t){{return taskNames[t].substring(0,6);}}),datasets:vlmModels.map(function(m,i){{return{{label:m,data:multiTasks.map(function(t){{var r=D.find(function(x){{return x.model.label===m&&x.task.id===t;}});return r&&r.ok&&r.output.usage?r.output.usage.total_tokens:0;}}),backgroundColor:vlmColors[i%2]+"88"}};}})}},
 options:{{scales:{{y:{{title:{{display:true,text:"tokens",color:"#c9d1d9"}},ticks:{{color:"#c9d1d9"}},grid:{{color:"#30363d"}}}},x:{{ticks:{{color:"#c9d1d9"}}}}}},plugins:{{legend:{{labels:{{color:"#c9d1d9"}}}}}}}}
}});
...
rh += "</table><p style='margin-top:8px;color:#8b949e'>Text tasks and multimodal tasks are routed separately. Pure text models cannot handle multimodal.</p>";
...
print(f"Dashboard v2 regenerated!")
print(f"Path: {OUT / 'Dashboard.html'}")
print(f"Changes: fixed bar charts - text/multimodal split, added token charts, VLM-only multimodal comparison")
